<html>
 <head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
  <title>Histórico de Depósitos – EMI</title>
  <link rel="stylesheet" href="/v2/layui/css/layui.css">
  <link rel="stylesheet" href="/v2/css/common.css">
  <link rel="stylesheet" href="/v2/css/emi-theme.css">
  <style>
    .layui-colla-title {
      font-family: Arial, Arial;
      font-weight: 700;
      font-size: 16px;
      color: #1C1C1C;
    }
    .layui-colla-title .success { color: #A3B18A; }
    .layui-colla-title .progress { color: #C8A96A; }
    .layui-colla-title .returned { color: #D6A86B; }
    .layui-colla-content, .layui-colla-item {
      border-top: 1px solid #EDE8DF;
    }
    .padding { padding: 10px 0; }
    .bottom_order { border-bottom: 1px solid #EDE8DF; }
    .label { color: #666666 !important; }
    .value { color: #1C1C1C !important; font-weight: 700; }
  </style>
 </head>
 <body class="common_background" style="background-image: url(/v2/img/order/bg.png);">
  <div class="common_header">
   <a href="javascript:history.back(-1)" class="back position">
    <p class="btn"><i class="layui-icon layui-icon-left layui-font-20"></i></p>
    Histórico de Depósitos
   </a>
  </div>
  <div style="margin: 15px">
   <div class="common_card" style="background: #FFFFFF;">
    <p class="value" style="font-size: 18px; color: #1C1C1C; line-height: 21px;">{{ price(auth()->user()->balance) }}</p>
    <p class="label" style="font-size: 14px; color: #444444; line-height: 21px;">Saldo da Conta</p>
   </div>
   <div style="margin-top: 20px;">
    @foreach(\App\Models\Deposit::where('user_id', auth()->id())->orderByDesc('id')->get() as $element)
    @php
      $statusColor = match($element->status) {
        'approved' => '#A3B18A',
        'progress' => '#C8A96A',
        'rejected' => '#B05030',
        default => '#D6A86B'
      };
      $statusLabel = match($element->status) {
        'approved' => 'Aprovado',
        'progress' => 'Processando',
        'rejected' => 'Rejeitado',
        default => 'Pendente'
      };
    @endphp
    <a href="/rechargeDetails/{{ $element->id }}" class="common_card position" style="display:block">
     <div class="flex_space padding bottom_order">
      <p class="label">Status do Depósito</p>
      <p class="value"><span style="color: {{ $statusColor }}">{{ $statusLabel }}</span></p>
     </div>
     <div class="flex_space padding bottom_order">
      <p class="label">Valor do Depósito</p>
      <p class="value position">{{ price($element->amount) }}</p>
     </div>
     <div class="flex_space padding bottom_order">
      <p class="label">Valor Recebido</p>
      <p class="value position">{{ price($element->final_amount) }}</p>
     </div>
     <div class="flex_space padding bottom_order">
      <p class="label">Taxa</p>
      <p class="value position">{{ price($element->charge_amount) }}</p>
     </div>
     <div class="flex_space padding bottom_order">
      <p class="label">Data de Solicitação</p>
      <p class="value position">{{ $element->created_at }}</p>
     </div>
    </a>
    @endforeach
   </div>
  </div>
 </body>
</html>
